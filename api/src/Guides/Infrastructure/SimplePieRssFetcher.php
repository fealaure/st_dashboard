...
use SaveState\Guides\Domain\FetchedGuide;
...
final class SimplePieRssFetcher implements RssFetcher
{
    ...
    public function fetch(GuideSource $source): Generator
    {
        $feed = new SimplePie();
        ...
        try {
            ...
            $this->logger->error('Guide RSS fetch threw', [
                'source' => $source->slug,
                'error' => $e->getMessage(),
            ]);
        ...
        if (! $success) {
            $this->logger->warning('Guide RSS feed could not be parsed', [
                'source' => $source->slug,
                'error' => $feed->error(),
            ]);
